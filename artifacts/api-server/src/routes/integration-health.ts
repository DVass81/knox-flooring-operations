...
router.get("/integrations/health", async (_req, res) => {
  ...
  res.json({
    ...
    integrations: [
      ...
      { key: "quickbooks", name: "QuickBooks Online", status: quickbooks?.status === "connected" ? (process.env.QUICKBOOKS_ENVIRONMENT === "production" ? "Live" : "Sandbox") : "Disconnected", detail: quickbooks?.companyName ?? "Connect an Intuit sandbox company" },

...
router.post("/auth/demo", async (req, res) => {
  ...
  res.json({
    user: { id: owner.id, email: owner.email, name: owner.name, role: owner.role, actualRole: owner.role, previewRole: null },
...
router.post("/auth/login", async (req, res) => {
  ...
  res.json({ user: { id: user.id, email: user.email, name: user.name, role: user.role, actualRole: user.role, previewRole: null }, csrfToken: session.csrfToken });
});

  ...
  return (
    <header className="h-16 border-b bg-background/80 backdrop-blur-sm flex items-center justify-between px-4 md:px-8 sticky top-0 z-20">
      ...
      <div className="flex items-center gap-4">
        ...
        <Button variant="ghost" size="sm" className="hidden md:flex" title="Open Training Center" onClick={() => window.dispatchEvent(new Event("knox:demo-center"))}><BookOpen className="mr-1.5 h-4 w-4" />Training</Button>
        <Button variant="ghost" size="icon" className="md:hidden" title="Open Training Center" aria-label="Open Training Center" onClick={() => window.dispatchEvent(new Event("knox:demo-center"))}><BookOpen className="h-4 w-4" /></Button>

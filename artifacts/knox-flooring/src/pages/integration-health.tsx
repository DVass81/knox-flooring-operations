...
type Integration = { key: string; name: string; status: "Live" | "Sandbox" | "Simulated" | "Disconnected" | "Error"; detail: string };
...
export default function IntegrationHealth() {
  ...
  return <div className="mx-auto max-w-5xl space-y-6">
    ...
    <div className="grid gap-4 md:grid-cols-2">{items.map((item) => { const Icon = icons[item.key] || Activity; return <Card key={item.key}><CardHeader className="flex-row items-start justify-between space-y-0"><div className="flex gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-primary/10 text-primary"><Icon className="h-5 w-5" /></div><div><CardTitle className="text-base">{item.name}</CardTitle><CardDescription className="mt-1">{item.detail}</CardDescription></div></div><Badge className={styles[item.status]}>{item.status}</Badge></CardHeader><CardContent><div className="h-1.5 overflow-hidden rounded-full bg-muted"><div className={`h-full rounded-full ${item.status === "Live" ? "w-full bg-emerald-500" : item.status === "Sandbox" ? "w-4/5 bg-sky-500" : item.status === "Simulated" ? "w-3/5 bg-amber-500" : "w-1/5 bg-muted-foreground/30"}`} /></div></CardContent></Card>})}</div>
